...
if st.session_state.uploaded_file is not None:
                    ...
                    for file_path, file_content in message["files"].items():
                        try:
                            if isinstance(file_content, str) and file_content.startswith("data:"):
                                ...
                                file_data = (
                                    base64.b64decode(file_content) if isinstance(file_content, str) else file_content
                                )
    ...
    if prompt := st.chat_input("Ask the data analyst anything about your data..."):
        ...
        with st.chat_message("assistant"):
            if st.session_state.streaming_enabled:
                ...
                try:
                    ...
                    analysis_result, generated_files = analyze_csv_file(
                        st.session_state.uploaded_file, prompt, streaming_enabled=False
                    )
            else:
        ...
        st.session_state.messages.append({"role": "assistant", "content": analysis_result, "files": generated_files})
