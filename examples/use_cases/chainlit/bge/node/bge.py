...
from examples.components.rag.rerankers.bge.component.document import DocumentRanker as DocumentRankerComponent
...
class BGEDocumentRanker(Node):
    ...
    def init_components(self, connection_manager: ConnectionManager | None = None) -> None:
        ...
        if self.ranker is None:
            self.ranker = DocumentRankerComponent(model_name=self.model_name, threshold=self.threshold)
    ...
    def execute(self, input_data: dict[str, Any], config: RunnableConfig = None, **kwargs) -> dict[str, Any]:
        ...
        ranked_documents = self.ranker.run(query=input_data["query"], documents=input_data["documents"])
