...
class SearchEngineTool(Node):
    ...
    description: str = "A tool that combines web searching, content scraping, and summarization."
    search_tool: ConnectionNode = Field(None, description="The tool that performs the search.")
    scrape_tool: ConnectionNode = Field(None, description="The tool that scrapes the contents of the search results.")
    summarize_tool: Node = Field(None, description="The tool that summarizes the scraped contents.")
    default_limit: int = Field(5, description="The default limit for the number of search results.")
    ...
    def to_dict(self, **kwargs) -> dict:
        ...
        data["search_tool"] = self.search_tool.to_dict(**kwargs) if self.search_tool else None
        data["scrape_tool"] = self.scrape_tool.to_dict(**kwargs) if self.scrape_tool else None
        data["summarize_tool"] = self.summarize_tool.to_dict(**kwargs) if self.summarize_tool else None
        return data
    ...
    def init_components(self, connection_manager: ConnectionManager | None = None):
        ...
        if self.search_tool is not None and self.search_tool.is_postponed_component_init:
            self.search_tool.init_components(connection_manager)
        if self.scrape_tool is not None and self.scrape_tool.is_postponed_component_init:
            self.scrape_tool.init_components(connection_manager)
        if self.summarize_tool is not None and self.summarize_tool.is_postponed_component_init:
    ...
    def execute(self, input_data: dict[str, Any], config: RunnableConfig | None = None, **kwargs) -> dict[str, Any]:
        ...
        try:
            ...
            formatted_results = self._format_results(search_results["content"]["urls"], summaries)
            ...
            result = {
                ...
                "sources_with_url": search_results["content"].get("sources_with_url", []),
                "summaries": summaries,
            }
    ...
    def _scrape_contents(
        ...
        **kwargs,
        ...
        logger.debug(f"Tool {self.name} - {self.id}: scraped {len(scraped_contents)} pages")
    ...
    def _summarize_contents(
        ...
        **kwargs,
        ...
        logger.debug(f"Tool {self.name} - {self.id}: generated {len(summaries)} summaries")
        return summaries
