...
class DocumentRanker:
    ...
    def run(self, query: str, documents: list[Document]) -> list[Document]:
        ...
        selected_documents = [document for document, score in ranked_documents if score > self.threshold][: self.top_k]
        logger.debug(f"Selected {len(selected_documents)} documents with score > {self.threshold}")
