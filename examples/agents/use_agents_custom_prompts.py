...
from dynamiq.prompts import Message, VisionMessage, MessageRole
# Constants
...
def run_workflow() -> tuple[str, dict]:
    ...
    agent = ReflectionAgent(
        ...
        id="agent",
        verbose=True,
        context_message = Message(role = MessageRole.SYSTEM, content = "{{context}}"),
        input_message = Message(role = MessageRole.USER, content = "{{input1}}")
    )
    ...
    try:
        result = wf.run(
            input_data={"input1": INPUT_QUESTION, "context": "No additional context"},
