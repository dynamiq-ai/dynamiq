...
class BedrockAgentCoreSandbox(Sandbox):
    ...
    def _ensure_directories(self) -> None:
        ...
        try:
            ...
            if result.is_error or result.exit_code != 0:
                logger.warning(f"BedrockAgentCoreSandbox failed to create directory: {result.error_text}")
            else:
    ...
    def exists(self, file_path: str) -> bool:
        ...
        try:
            ...
            result = self._get_client().invoke(
                session, "executeCommand", {"command": f"test -f {shlex.quote(resolved_path)}"}
            )
