...
class MilvusDocumentRetriever:
    ...
    def __init__(
        ...
        *,
        ...
        This initializer checks if the `vector_store` provided is an instance of the expected `MilvusVectorStore`
        ...
        if not isinstance(vector_store, MilvusVectorStore):
            msg = "document_store must be an instance of MilvusVectorStore"
    ...
    def run(
        ...
        """
        Retrieves documents from the MilvusDocumentStore that are similar to the provided query embedding.
        ...
        logger.debug(f"Retrieved {len(docs)} documents from Milvus Vector Store.")
