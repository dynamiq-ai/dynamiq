...
THROTTLING_ERROR_CODES = ("ThrottlingException", "TooManyRequestsException", "ThrottledException")


class AgentCoreThrottlingError(Exception):
    """Raised when AWS throttles Bedrock AgentCore requests."""
...
class AgentCoreCodeInterpreterClient:
    ...
    def __init__(self, connection: AWSConnection, read_timeout: int = 910):
        ...
        self._client = session.client("bedrock-agentcore", config=Config(read_timeout=read_timeout))
    ...
    def invoke(self, session: AgentCoreSession, tool_name: str, arguments: dict) -> AgentCoreInvocationResult:
        try:
            ...
            for event in response["stream"]:
                ...
                if event_result.get("structuredContent"):
                    result.structured = event_result["structuredContent"]
