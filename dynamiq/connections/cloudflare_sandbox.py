...
class CloudflareSandboxClient:
    ...
    def _validate_sandbox_id(sandbox_id: str) -> str:
        if not SANDBOX_ID_PATTERN.match(sandbox_id or ""):
    ...
    def _request(self, method: str, path: str, *, read_timeout: float | None = None, **kwargs) -> requests.Response:
        ...
        response = self._session.request(method, self._url(path), timeout=timeout, **kwargs)
    ...
    def _parse_exec_sse(response: requests.Response) -> CloudflareExecResult:
        ...
        def handle_event() -> None:
            ...
            if event in ("stdout", "stderr"):
                try:
                    ...
                    decoded = data
                (stdout_parts if event == "stdout" else stderr_parts).append(decoded)
            elif event == "exit":
                try:
        ...
        return CloudflareExecResult(exit_code=exit_code, stdout="".join(stdout_parts), stderr="".join(stderr_parts))

    def _file_url_path(self, sandbox_id: str, path: str) -> str:
        resolved = resolve_workspace_path(path)
        return f"/sandbox/{self._validate_sandbox_id(sandbox_id)}/file{quote(resolved)}"
