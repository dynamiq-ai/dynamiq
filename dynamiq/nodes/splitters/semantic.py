...
class SemanticChunker(Node):
    ...
    name: str = "SemanticChunker"
    description: str = "Splits text on semantic-similarity breakpoints between sentence groups."
    ...
    buffer_size: int = Field(default=1, ge=0, description="Number of neighbour sentences in each embedding group.")
    sentence_split_regex: str = Field(
        default=r"(?<=[.?!])\s+",
        description="Regex used to split text into sentences before grouping.",
    )
    min_chunk_size: int = Field(
        default=0, ge=0, description="Merge tail chunks shorter than this back into the previous chunk."
    )

    splitter: Any | None = None
    input_schema: ClassVar[type[SemanticChunkerInputSchema]] = SemanticChunkerInputSchema
    ...
    def init_components(self, connection_manager: ConnectionManager | None = None) -> None:
        ...
        if self.embedder is None:
            raise ValueError("SemanticChunker requires an `embedder` (TextEmbedder) node.")
        self.embedder.init_components(connection_manager)
        if self.splitter is None:
            self.splitter = SemanticChunkerComponent(
    ...
    def execute(
        self, input_data: SemanticChunkerInputSchema, config: RunnableConfig = None, **kwargs
    ) -> dict[str, Any]:
        config = ensure_config(config)
        self.run_on_node_execute_run(config.callbacks, **kwargs)
        documents = input_data.documents
        logger.debug(f"SemanticChunker: splitting {len(documents)} documents.")
