...
class ParallelToolCallsInputSchema(BaseModel):
    ...
    Example:
        {
            "tools": [
                {"name": "search_tool", "input": {"query": "AI news"}},
                {"name": "weather_tool", "input": {"city": "NYC"}}
            ]
        }
    """
    ...
    tools: list[dict[str, Any]] = Field(
        ...,
        description=(
            "List of tools to execute in parallel. Each item must have 'name' (tool name) "
            "and 'input' (dict of tool parameters). Example: "
            "[{\"name\": \"search\", \"input\": {\"query\": \"AI\"}}, "
            "{\"name\": \"calc\", \"input\": {\"expr\": \"2+2\"}}]"
        ),
        min_length=1,
    )
...
class ParallelToolCallsTool(Node):
    ...
    name: str = "Parallel Tool Calls"
    description: str = (
        """Execute multiple tools simultaneously in parallel.
...
{
    "tools": [
        {"name": "<tool_name>", "input": {<tool_parameters>}},
        {"name": "<another_tool>", "input": {<tool_parameters>}}
    ]
}

Example - search two topics at once:
{
    "tools": [
        {"name": "search", "input": {"query": "Python best practices"}},
        {"name": "search", "input": {"query": "Python performance tips"}}
    ]
}"""
    )
