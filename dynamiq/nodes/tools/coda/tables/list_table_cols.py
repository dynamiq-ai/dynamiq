...
class ListColumnsInputSchema(BaseModel):
    """
    Input schema for listing columns in a  table.
    """
...
class ListTableColumns(BaseCoda):
    """
    Node to list columns in a specific table of a  doc.
    """

    name: str = "ListColumns"
    description: str = "Fetch a list of columns from a table in a  doc."
    input_schema: ClassVar[type[ListColumnsInputSchema]] = ListColumnsInputSchema

    def execute(self, input_data: ListColumnsInputSchema, config: RunnableConfig | None = None, **kwargs):
