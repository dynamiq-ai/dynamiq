...
from ..base import BaseCoda, CellEdit


class UpdateRowInputSchema(BaseModel):
    """
    Input schema for updating a single row in a  table.
    """
...
class UpdateRow(BaseCoda):
    """
    Node to update an existing row in a  table.
    """

    name: str = "UpdateRow"
    description: str = "Update a specific row in a  table."
    input_schema: ClassVar[type[UpdateRowInputSchema]] = UpdateRowInputSchema

    def execute(self, input_data: UpdateRowInputSchema, config: RunnableConfig | None = None, **kwargs):
