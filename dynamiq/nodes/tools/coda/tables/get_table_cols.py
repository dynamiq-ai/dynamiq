...
class GetColumnInputSchema(BaseModel):
    """
    Input schema for fetching details about a single column in a  table.
    """
...
class GetTableColumn(BaseCoda):
    """
    Node to retrieve details about a single column in a  table.
    """

    name: str = "GetColumn"
    description: str = "Get details about a specific column in a  table."
    input_schema: ClassVar[type[GetColumnInputSchema]] = GetColumnInputSchema

    def execute(self, input_data: GetColumnInputSchema, config: RunnableConfig | None = None, **kwargs):
