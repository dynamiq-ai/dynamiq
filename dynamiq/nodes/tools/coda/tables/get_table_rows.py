...
class GetRowInputSchema(BaseModel):
    """
    Input schema for fetching details about a single row in a  table.
    """
...
class GetTableRows(BaseCoda):
    """
    Node to retrieve details about a specific row in a  table.
    """

    name: str = "GetRow"
    description: str = "Retrieve details about a specific row in a  table."
    input_schema: ClassVar[type[GetRowInputSchema]] = GetRowInputSchema

    def execute(self, input_data: GetRowInputSchema, config: RunnableConfig | None = None, **kwargs):
