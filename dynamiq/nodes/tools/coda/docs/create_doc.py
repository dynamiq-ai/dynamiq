...
from ..base import BaseCoda, PageContentUnion
...
class CreateDocInputSchema(BaseModel):
    """
    Input schema for creating a new  doc.
    """
...
class CreateDoc(BaseCoda):
    """
    Node to create a new  doc.
    """

    name: str = "CreateDoc"
    description: str = "Create a new  doc with optional parameters."
    input_schema: ClassVar[type[CreateDocInputSchema]] = CreateDocInputSchema

    def execute(self, input_data: CreateDocInputSchema, config: RunnableConfig | None = None, **kwargs):
        """
        Execute the node to create a new  doc.

        Args:
            input_data (CreateDocInputSchema): Input schema data.
