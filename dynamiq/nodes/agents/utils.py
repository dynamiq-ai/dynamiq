...
def first_xml_output_block(text: str) -> str:
    """Return only the first ``<output>...</output>`` block (the XML turn boundary).

    ``</output>`` is the agent's stop sequence, but the API strips ``stop`` for reasoning models
    (o-series / gpt-5), so they can run past it and emit a second ``<output>`` block that fabricates
    an ``<answer>`` before the tool ran. Keeping only the first block enforces that boundary
    client-side. Used by both the parse path (``Agent._first_output_block``) and the live stream
    (``AgentStreamingParserCallback``) so there is one definition of the boundary.

    Free-form tag content (``action_input``/``thought``/``answer``/``output_files``) is treated as
    opaque so a ``</output>`` inside code, JSON or a filename is not mistaken for the boundary.
    ...
    while i < n:
        ...
        for tag in opaque_tags:
            ...
            if text.startswith(open_tag, i) and (i + len(open_tag) >= n or text[i + len(open_tag)] in " >/"):
                close = f"</{tag}>"
                ci = text.find(close, i)
                i = ci + len(close) if ci != -1 else n
                matched = True
                break
        if matched:
            continue
