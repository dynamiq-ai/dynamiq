    ...
    def _run_agent(self, config: RunnableConfig | None = None, **kwargs) -> str:
            ...
            try:
                ...
                match self.inference_mode:
                    case InferenceMode.DEFAULT:
                        ...
                        if self.streaming.enabled and self.streaming.mode == StreamingMode.ALL:
                            ...
                            self.stream_chunk(
                                input_chunk=self._prepare_stream_result(llm_generated_output),
                                config=config,
                                **kwargs,
                            )
                        if "Answer:" in llm_generated_output:
                            ...
                            if self.streaming.enabled and self.streaming.mode == StreamingMode.FINAL:
                                self.stream_chunk(
                                    input_chunk=self._prepare_stream_result(final_answer),
                                    config=config,
                                    **kwargs,
                                )
                            return final_answer
                ...
                if action:
                    ...
                    if self.tools:
                        ...
                        if self.streaming.enabled and self.streaming.mode == StreamingMode.ALL:
                            self.stream_chunk(
                                input_chunk=self._prepare_stream_result(observation),
                                config=config,
                                **kwargs,
                            )
        ...
        if self.behaviour_on_max_loops == Behavior.RAISE:
            ...
            if self.streaming.enabled:
                self.stream_chunk(
                    input_chunk=self._prepare_stream_result(max_loop_final_answer),
                    config=config,
                    **kwargs,
                )
