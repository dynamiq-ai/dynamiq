...
class Agent(Node):
    ...
    def generate_function_calling_schemas(self):
        ...
        for tool in self.tools:
            ...
            for name, field_def in tool.input_schema.model_fields.items():
                if field_def.json_schema_extra and not field_def.json_schema_extra.get("is_accessible_to_agent", True):
                    continue
    ...
    def _run_agent(
        ...
        **kwargs,
        ...
        if not self.tools or self.max_loops <= 1:
            logger.info(f"{self.name}: Using single-shot approach (no tools or max_loops=1).")
            ...
            final_text = llm_result.output.get("content", "")
            if self.streaming.enabled:
                self.stream_content(final_text, source=self.name, step="final_answer", config=config)
            return final_text

        logger.info(
            f"{self.name}: Using multi-step approach with max_loops={self.max_loops} and {len(self.tools)} tools."
        )
        ...
        for loop_idx in range(1, self.max_loops + 1):
            ...
            self._intermediate_steps[loop_idx] = AgentIntermediateStep(
                ...
                model_observation=AgentIntermediateStepModelObservation(initial=output_text),
            ).model_dump()
            ...
            if self.inference_mode == InferenceMode.FUNCTION_CALLING and tool_calls:
                for call_id, call_info in tool_calls.items():
                    ...
                    thought = func_args["thought"]
                    action_input = func_args["action_input"]
    ...
    def _log_and_stream_final(self, loop_idx: int, final_answer: str, config: RunnableConfig, **kwargs):
        ...
        """
        logger.info(f"{self.name}: Loop={loop_idx} => Final answer:\n{final_answer}")
