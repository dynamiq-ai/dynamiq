...
def _normalize_filters(filters: dict[str, Any]) -> dict[str, Any]:
    """
    Converts filters to OpenSearch compatible filters.

    Args:
        filters (dict[str, Any]): The filters to be normalized.

    Returns:
        dict[str, Any]: Normalized filters compatible with OpenSearch.
...
def _equal(field: str, value: Any) -> dict[str, Any]:
    ...
    if not isinstance(value, supported_types):
        msg = (
            ...
            f"Types supported by OpenSearch are: {supported_types}"
        )
        raise VectorStoreFilterException(msg)
