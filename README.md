...
### RAG - document indexing with Elasticsearch
This workflow demonstrates how to use Elasticsearch as a vector store for RAG applications. It includes document conversion, embedding generation, and storage in Elasticsearch with vector search capabilities.
...
def create_indexing_workflow(files: List[str]) -> None:
    ...
    es_connection = ElasticsearchConnection(
        ...
        password=os.getenv("ELASTICSEARCH_PASSWORD", "your-password"),
        use_ssl=True,
        verify_certs=False,
    )
...
# Example usage
files = ["example.pdf"]  # Replace with your PDF files
create_indexing_workflow(files)
```

### RAG - document retrieval with Elasticsearch
...
def create_retrieval_workflow(question: str) -> str:
    ...
    document_retriever = (
        ...
        .inputs(
            ...
            search_type="dense",  # Use dense vector search
...
### RAG - hybrid search with Elasticsearch
This example demonstrates Elasticsearch's hybrid search capabilities, combining dense vector similarity with text matching for more accurate document retrieval.
...
def create_hybrid_indexing_workflow(files: List[str]) -> None:
    """Create and run the document indexing workflow with optimized settings for hybrid search."""
    ...
    # Elasticsearch vector store with optimized settings for hybrid search
    vector_store = (
        ElasticsearchDocumentWriter(
            connection=es_connection,
            ...
            similarity="cosine",
            create_if_not_exist=True,
            # Index settings optimized for both vector and text search
            index_settings={
                "number_of_shards": 1,
                "number_of_replicas": 1,
                "analysis": {
            ...
            # Mapping settings for hybrid search
            mapping_settings={
                "dynamic": "strict",
                "properties": {
                    "content": {
            ...
            # Optional vector quantization for efficiency
            quantization_config={
    ...
    # Run indexing workflow
    rag_wf.run(input_data=input_data)
    print(f"Successfully indexed {len(files)} documents with hybrid search configuration")


def create_hybrid_retrieval_workflow(question: str, dense_weight: float = 0.7) -> str:
    """Create and run the document retrieval workflow using hybrid search."""
    ...
    # Elasticsearch retriever with hybrid search
    document_retriever = (
        ElasticsearchDocumentRetriever(
            connection=es_connection,
            index_name="documents_hybrid",
        ...
        .inputs(
            ...
            scale_scores=True,  # Scale scores to 0-1 range
            dense_weight=dense_weight,  # Weight for dense vector scores (0-1)
        )
        .depends_on(embedder)
    )
    retrieval_wf.flow.add_nodes(document_retriever)

    # Answer generation with context about search method
    prompt_template = """
    Please answer the question based on the provided context. The context was retrieved using a hybrid search method
    that combines semantic similarity (dense vectors) and keyword matching (sparse vectors).
    ...
    answer_generator = (
        OpenAI(
            ...
            model="gpt-4o",
            prompt=prompt,
            temperature=0.7,  # Slightly higher temperature for more nuanced answers
        )
        .inputs(
            documents=document_retriever.outputs.documents,
            query=embedder.outputs.query,
...
# Index documents with hybrid search configuration
create_hybrid_indexing_workflow(files)
...
# Try different weights between dense and sparse search
weights = [0.3, 0.5, 0.7]

for question in questions:
    print(f"\nQuestion: {question}")
    for weight in weights:
        print(f"\nUsing dense_weight={weight}:")
        answer = create_hybrid_retrieval_workflow(question, dense_weight=weight)
        print(f"Answer: {answer}")
```
