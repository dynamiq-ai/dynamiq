...
try:  # pragma: no cover - environment probe
    ...
    _requests.get(QDRANT_URL, timeout=1)
    QDRANT_AVAILABLE = True
except Exception:
    QDRANT_AVAILABLE = False

pytestmark = pytest.mark.skipif(
    not QDRANT_AVAILABLE, reason=f"Qdrant not reachable at {QDRANT_URL}"
)
...
def test_qdrant_insert_then_get(backend, fake_embedder):
...
def test_qdrant_get_unknown_returns_none(backend):
    assert backend.get("does-not-exist") is None


def test_qdrant_get_by_hash(backend, fake_embedder):
...
def test_qdrant_get_by_hash_isolates_users(backend, fake_embedder):
    backend.insert(_fact("f1", "u1", "x", "h-shared"), fake_embedder.embed("x"))
    assert backend.get_by_hash(user_id="u2", content_hash="h-shared") is None


def test_qdrant_metadata_round_trip(backend, fake_embedder):
...
def test_qdrant_delete(backend, fake_embedder):
    backend.insert(_fact("f1", "u1", "x"), fake_embedder.embed("x"))
    backend.delete("f1")
    assert backend.get("f1") is None


def test_qdrant_list_by_scope(backend, fake_embedder):
...
def test_qdrant_delete_scope(backend, fake_embedder):
    backend.insert(_fact("f1", "u1", "a"), fake_embedder.embed("a"))
    backend.insert(_fact("f2", "u1", "b"), fake_embedder.embed("b"))
    backend.insert(_fact("f3", "u2", "c"), fake_embedder.embed("c"))
...
def test_qdrant_search_relevance_ordered(backend, fake_embedder):
...
def test_qdrant_search_isolates_users(backend, fake_embedder):
    backend.insert(_fact("f1", "u1", "alpha"), fake_embedder.embed("alpha"))
...
def test_qdrant_search_empty_returns_empty(backend, fake_embedder):
    hits = backend.search(
