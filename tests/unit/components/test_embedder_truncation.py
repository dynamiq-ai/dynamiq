...
def test_truncation_logic_integration():
    ...
    long_text = "A" * 100
    result = truncate_text_for_embedding(long_text, max_tokens=10, truncate_method=TruncationMethod.MIDDLE)
...
def test_truncation_with_different_methods_integration():
    ...
    result_start = truncate_text_for_embedding(long_text, max_tokens=20, truncate_method=TruncationMethod.START)
    assert result_start.startswith("...[truncated for embedding]...")
    assert result_start.endswith("X")

    result_end = truncate_text_for_embedding(long_text, max_tokens=20, truncate_method=TruncationMethod.END)
    assert result_end.startswith("X")
    assert result_end.endswith("...[truncated for embedding]...")

    result_middle = truncate_text_for_embedding(long_text, max_tokens=20, truncate_method=TruncationMethod.MIDDLE)
...
def test_embedder_has_truncation_configuration():
    ...
    assert "enable_truncation" in model_fields
    assert "max_input_tokens" in model_fields
    assert "truncation_method" in model_fields

    assert model_fields["enable_truncation"].default is True
