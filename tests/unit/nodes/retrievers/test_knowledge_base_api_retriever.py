...
from dynamiq.nodes.retrievers.knowledge_base import (
    KnowledgeBaseAPIRetriever,
    KnowledgeBaseAPIRetrieverInputSchema,
)
...
def retriever(connection):
    node = KnowledgeBaseAPIRetriever(connection=connection, knowledge_base_id="kb-123", top_k=5)
...
def test_type_resolves_to_module_path(retriever):
    assert retriever.type == "dynamiq.nodes.retrievers.KnowledgeBaseAPIRetriever"
...
def test_build_url(retriever):
    assert retriever._build_url() == "https://api.example.ai/v1/knowledgebases/kb-123/search"
...
def test_execute_builds_request_and_parses_documents(retriever):
    ...
    input_data = KnowledgeBaseAPIRetrieverInputSchema(query="hello", filters={"k": "v"}, top_k=3)
    ...
    assert kwargs["url"] == "https://api.example.ai/v1/knowledgebases/kb-123/search"
...
def test_execute_uses_node_defaults_for_top_k(retriever):
    ...
    retriever.execute(KnowledgeBaseAPIRetrieverInputSchema(query="q"), RunnableConfig(callbacks=[]))
...
def test_execute_content_falls_back_to_joined_documents(retriever):
    ...
    result = retriever.execute(KnowledgeBaseAPIRetrieverInputSchema(query="q"), RunnableConfig(callbacks=[]))
...
async def test_execute_async_builds_request_and_parses_documents(retriever):
    ...
    with patch.object(KnowledgeBaseAPIRetriever, "get_async_client", AsyncMock(return_value=async_client)):
        result = await retriever.execute_async(
            KnowledgeBaseAPIRetrieverInputSchema(query="hi"), RunnableConfig(callbacks=[])
        )
