...
@pytest.mark.parametrize("column_type", ["Any", "float"])
def test_a_yaml_float_cell_in_exponent_form_reads_as_the_number(column_type):
    # A YAML source holds the cell as the float 0.00001, which Python writes back as `1e-05`.
    table = DecisionTable(
        input_columns=[NamedField(id="conc", name="conc", type=column_type)],
        output_columns=[NamedField(id="verdict", name="verdict", type="string")],
        rules=[
            DecisionRule(id="r1", name="trace", when=[0.00001], then=["trace"]),
            DecisionRule(id="r2", name="band", when=["[1e-4..1e-2)"], then=["band"]),
            DecisionRule(id="r3", name="rest", when=["> 1E-2"], then=["rest"]),
        ],
    )

    assert run_node(table, {"conc": 0.00001}).output["verdict"] == "trace"
    assert run_node(table, {"conc": 0.001}).output["verdict"] == "band"
    assert run_node(table, {"conc": 1e6}).output["verdict"] == "rest"
    assert run_node(table, {"conc": 0.00002}).output["verdict"] is None
    # A numeric column reads numeric text in either form; an Any column keeps the text as it is.
    assert (run_node(table, {"conc": "1e6"}).output["verdict"] == "rest") is (column_type == "float")
